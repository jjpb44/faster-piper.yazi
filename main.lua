...
function M:preload(job)
  -- ponytail: skip warming huge files — hover must stay cheap; preview still
  -- generates on demand. Raise the ceiling when disks/CPUs make it cheap.
  local len = job.file.cha and job.file.cha.len or 0
  if len > 8 * 1024 * 1024 then
    return false
  end
